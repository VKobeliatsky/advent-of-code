...
computePowerRates = collectCounts >>> collectRates >>> bimap binToInt binToInt
...
collectRates (rows, counts) = foldl'
  (\(!gamma, !epsilon) !count -> if count > rows `div` 2
...
computeScrubberRates = tableFromString
  >>> (\table
       -> (table & findO2Rating & binToInt, table & findCO2Rating & binToInt))
...
findCO2Rating = go 0
  where
    go colIx rows
      ...
      | otherwise = let (rows0, rows1) = partition (at colIx >>> (==) '0') rows
                    in go (colIx + 1)
                       $ if Vector.length rows0 <= Vector.length rows1
                         then rows0
                         else rows1
